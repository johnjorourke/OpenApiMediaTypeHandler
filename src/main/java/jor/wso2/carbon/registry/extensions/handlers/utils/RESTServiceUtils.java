...
public class RESTServiceUtils {
	...
	public static OMElement createRestServiceArtifact(JsonObject swaggerDocObject, String swaggerVersion,
		...
		if (StringUtils.isNotEmpty(swaggerVersion) && swaggerVersion.startsWith(SwaggerConstants.OPENAPI_VERSION_3)) {
			apiVersion.setText(getChildElementText(infoObject, org.wso2.carbon.registry.extensions.handlers.utils.SwaggerConstants.VERSION));
			transports.setText(jor.wso2.carbon.registry.extensions.handlers.utils.SwaggerConstants.JOR_DEFAULT_TRANSPORT);
			...
			apiVersion.setText(getChildElementText(infoObject, org.wso2.carbon.registry.extensions.handlers.utils.SwaggerConstants.VERSION));
			transports.setText(getChildElementText(swaggerDocObject, org.wso2.carbon.registry.extensions.handlers.utils.SwaggerConstants.SCHEMES));
			uriTemplates = createURITemplateFromSwagger2(swaggerDocObject);

		} else if (SwaggerConstants.SWAGGER_VERSION_12.equals(swaggerVersion)) {
			apiVersion.setText(getChildElementText(swaggerDocObject, org.wso2.carbon.registry.extensions.handlers.utils.SwaggerConstants.API_VERSION));
     ...
     * @param swaggerDocObject      swagger Json Object.
     * @param swaggerVersion        swagger version.
     * @param endpointURLs          collection of Endpoints of the swagger
     * @param resourceObjects       swagger resource object list.
     ...
     */
    public static OMElement createRestServiceArtifact(JsonObject swaggerDocObject, String swaggerVersion,
                                                      List<String> endpointURLs, List<JsonObject> resourceObjects, String swaggerPath, String documentVersion)
            throws RegistryException {
		...
		Iterator<String> endpointIterator = endpointURLs.iterator();
		while (endpointIterator.hasNext()) {
			endpoints += endpointIterator.next();
        ...
        if (SwaggerConstants.SWAGGER_VERSION_2.equals(swaggerVersion)) {
			...
			apiVersion.setText(documentVersion);
			transports.setText(SwaggerConstants.JOR_DEFAULT_TRANSPORT);
			uriTemplates = createURITemplateFromSwagger2(swaggerDocObject);
		}
